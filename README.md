...
Die Katalog-Skills stehen in [`index.json`](index.json), einschließlich der persönlichen Blog-Skills und `thinkport-blog-promote`. Die fachliche Quelle jedes Eintrags ist die verlinkte `SKILL.md`; zusätzliche Skill-Metadaten liegen jeweils in `metadata.json`.
