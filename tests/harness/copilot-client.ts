...
export class SkillCopilotClient implements CopilotClient {
  ...
  private skillDirByName: Map<string, string> | null = null;
  ...
  private getSkillDirectoryMap(): Map<string, string> {
    if (this.skillDirByName) {
      return this.skillDirByName;
    }
    ...
    if (existsSync(indexPath)) {
      try {
        ...
        const catalog = JSON.parse(content) as { skills?: Array<{ name?: string; path?: string }> };
        for (const skill of catalog.skills ?? []) {
          if (!skill.name || !skill.path) continue;
          const resolved = resolve(this.basePath, skill.path);
          map.set(skill.name, resolved.replace(/\/SKILL\.md$/u, ""));
        }
      } catch {
  ...
  loadSkillContext(skillName: string): string {
    ...
    const fromCatalog = this.getSkillDirectoryMap().get(skillName);
    const skillDir = fromCatalog ?? join(this.skillsDir, skillName);
