...
export class SkillEvaluationRunner {
  ...
  /**
   * Find the repository root by looking for local skill layout markers.
   */
  private findRepoRoot(): string {
    const cwd = process.cwd();

    // Check if we're in the tests directory
    const parentSkills = join(cwd, "..", "skills");
    const parentCatalog = join(cwd, "..", "index.json");
    if (existsSync(parentSkills) && existsSync(parentCatalog)) {
      return resolve(cwd, "..");
    }

    // Check if we're at the repo root
    const rootSkills = join(cwd, "skills");
    const rootCatalog = join(cwd, "index.json");
    if (existsSync(rootSkills) && existsSync(rootCatalog)) {
      return cwd;
    }
  ...
  /**
   * List all skills from root index.json catalog.
   */
  listCatalogSkills(): string[] {
    const catalogPath = join(this.basePath, "index.json");
    ...
    try {
      ...
      const catalog = JSON.parse(content) as { skills?: Array<{ name?: string }> };
  ...
  async run(
    ...
    return {
      ...
      status: results.length - passed === 0 ? "passed" : "failed",
    };
  }
...
function summaryToDict(summary: EvaluationSummary): Record<string, unknown> {
  const inferredStatus =
    summary.status ??
    (summary.evaluationSource === "catalog_only"
      ? "catalog_only"
      : summary.failed > 0
        ? "failed"
        : "passed");
  ...
  return {
    ...
    evaluation_source: summary.evaluationSource ?? "harness",
...
async function main(): Promise<number> {
  ...
  if (options.all) {
    const skills = runner.listCatalogSkills();
    const availableSkills = new Set(runner.listAvailableSkills());
    if (skills.length === 0) {
      console.log(chalk.red("No skills found in index.json catalog."));
      return 1;
    }
      ...
      try {
        if (!availableSkills.has(skillName)) {
          skillResults.push({
            ...
            durationMs: 0,
            results: [],
            evaluationSource: "catalog_only",
            status: "catalog_only",
          });
          passedSkills++;
          if (!options.verbose) {
            console.log(chalk.blue("○ catalog-only"));
          }
          continue;
        }
